...
data = {
    ...
    'number_of_purchases': [10, 12, 8, 14, 15, 10, 5, 13, 6, 9]
}
...
# Function to input new customer data and predict cluster
def predict_segment(new_customer_data):
    # Standardize the new customer data
    new_customer_data_scaled = scaler.transform([new_customer_data])
    
    # Predict the cluster
    cluster = kmeans.predict(new_customer_data_scaled)
    return cluster[0]

# Input new customer data
new_customer_data = [19, 80, 11]  # Example input features: annual income, spending score, number of purchases

# Predict the segment
segment_prediction = predict_segment(new_customer_data)
print(f"The new customer is assigned to segment: {segment_prediction}")
